from django.test import TestCase

from cricket.models import (
    Player,
    Team,
    Match,
    Innings,
)

from cricket.services.innings_service import InningsService


class InningsWicketsLimitTestCase(TestCase):

    def setUp(self):
        self.striker = Player.objects.create(
            player_id="WK1",
            name="Striker",
            email="wk1@test.com",
            role="batsman",
            batting_style="right-hand",
        )

        self.non_striker = Player.objects.create(
            player_id="WK2",
            name="Non Striker",
            email="wk2@test.com",
            role="batsman",
            batting_style="left-hand",
        )

        self.bowler = Player.objects.create(
            player_id="WKB",
            name="Bowler",
            email="wkb@test.com",
            role="bowler",
            batting_style="right-hand",
            bowling_type="seam",
            bowling_style="right-arm-fast",
        )

        self.team1 = Team.objects.create(
            team_id="WKT1",
            team_name="Batting Team",
            captain=self.striker,
        )

        self.team1.players.add(
            self.striker,
            self.non_striker,
        )

        self.team2 = Team.objects.create(
            team_id="WKT2",
            team_name="Bowling Team",
            captain=self.bowler,
        )

        self.team2.players.add(
            self.bowler,
        )

        self.match = Match.objects.create(
            match_id="WICKETLIMIT1",
            team1=self.team1,
            team2=self.team2,
            ground="Wicket Limit Ground",
            ball_type="tennis",
            overs=20,
            status="scheduled",
            toss_winner=self.team1,
            toss_decision="bat",
            batting_team=self.team1,
            bowling_team=self.team2,
        )

        self.innings = Innings.objects.create(
            innings_number=1,
            match=self.match,
            batting_team=self.team1,
            bowling_team=self.team2,
            overs=20,
            striker=self.striker,
            non_striker=self.non_striker,
            current_bowler=self.bowler,
        )

        self.service = InningsService()

    def record_wicket(self):
        return self.service.record_wicket(
            innings=self.innings,
            wicket_type="bowled",
            player_out_id=self.striker.player_id,
            runs=0,
        )

    def test_tenth_wicket_is_allowed(self):
        self.innings.wickets_fallen = 9

        self.innings.save(
            update_fields=["wickets_fallen"]
        )

        self.record_wicket()

        self.innings.refresh_from_db()

        self.assertEqual(
            self.innings.wickets_fallen,
            10
        )

    def test_cannot_record_wicket_after_10_wickets(self):
        self.innings.wickets_fallen = 10

        self.innings.save(
            update_fields=["wickets_fallen"]
        )

        with self.assertRaisesMessage(
            ValueError,
            "All 10 wickets have fallen. Innings is complete."
        ):
            self.record_wicket()